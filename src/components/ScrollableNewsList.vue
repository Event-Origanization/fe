<template>
  <div class="h-[600px] lg:h-0 lg:min-h-full flex flex-col bg-white/[0.02] border border-white/5 rounded-lg p-5" v-bind="$attrs">
    ...
    <!-- Loading -->
    <div v-if="postStore.loadingRecentNews" class="flex justify-center items-center w-full h-[50vh]">
      <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-red-500"></div>
    </div>
    <div v-else class="flex-1 overflow-y-auto custom-scrollbar pr-2 flex flex-col gap-4">
...
const items = computed<NewsItem[]>(() => {
  if (!postStore.recentNews) return []
