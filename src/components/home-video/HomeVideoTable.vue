<!-- src/components/home-video/HomeVideoTable.vue -->
<template>
  <div class="space-y-4">
    <!-- Filters & Search -->
        ...
        <table class="min-w-full">
          <thead>
            <tr class="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
              <th class="px-5 py-4 text-left font-semibold text-gray-600 dark:text-gray-300">{{ $t('HOME_VIDEO_ADMIN.TABLE.VIDEO') }}</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            <tr
              v-for="video in homeVideoStore.videos"
              :key="video.id"
              ...
              <td class="px-5 py-4">
                <div class="flex items-center gap-4">
                  <div class="h-12 w-20 rounded-lg bg-gray-900 border border-gray-700 flex-shrink-0 relative overflow-hidden flex items-center justify-center group cursor-pointer" @click="$emit('edit', video)">
                    ...
                    <template v-else>
                      <span class="text-xs text-white uppercase font-bold tracking-tighter opacity-50">Banner</span>
                    </template>
            ...
            <tr v-if="homeVideoStore.videos.length === 0">
