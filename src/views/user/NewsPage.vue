    ...
    <section class="relative pt-32 pb-20 flex flex-col items-center justify-center text-center border-b border-gray-100 overflow-hidden bg-[#f8fafc]">
      ...
      <div class="relative z-10 w-full max-w-7xl mx-auto px-4">
        ...
        <div class="flex items-center justify-center gap-2 text-sm md:text-base font-bold text-gray-400 uppercase tracking-widest" data-aos="fade-up" data-aos-delay="100">
          ...
          <span class="text-brand-600">TIN TỨC SỰ KIỆN</span>
        </div>
      </div>
    </section>
    ...
    <main class="container mx-auto px-4 py-20">
      ...
      <section class="mb-24" v-if="postStore.recentNews.length > 0">
        <div class="mb-10 text-center md:text-left flex flex-col md:flex-row md:items-end justify-between border-b border-gray-200 pb-4" data-aos="fade-right">
          <div>
            <h4 class="text-brand-600 font-bold uppercase tracking-widest text-sm mb-2">TIN TỨC MỚI NHẤT</h4>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div class="lg:col-span-7 group cursor-pointer" @click="goToPost(postStore.recentNews[0].slug)" data-aos="fade-up">
            <div class="w-full aspect-[4/3] rounded-[30px] overflow-hidden relative shadow-lg hover:shadow-[0_24px_60px_rgba(220,38,38,0.15)] mb-6 transition-shadow duration-500">
              ...
              <div class="absolute top-6 left-6 bg-brand-600 text-white text-xs font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-md">Hot News</div>
            ...
            <p class="text-gray-600 text-base line-clamp-3">Cập nhật những thông tin nóng hổi nhất về thị trường sự kiện và các hoạt động nổi bật của 5P Event.</p>
          </div>

          <!-- 2 Smaller Featured Posts -->
          <div class="lg:col-span-5 flex flex-col gap-8">
            <div v-for="(post, i) in postStore.recentNews.slice(1, 3)" :key="post.id" @click="goToPost(post.slug)" class="group cursor-pointer flex gap-4 md:gap-6 items-center bg-white border border-gray-100 p-4 rounded-[24px] shadow-sm hover:shadow-[0_12px_40px_rgba(220,38,38,0.1)] hover:border-brand-200 hover:-translate-y-1 transition-all duration-400" data-aos="fade-left" :data-aos-delay="i * 100 + 100">
            ...
            <div class="mt-auto bg-brand-50 rounded-[24px] p-6 border border-brand-100 text-center flex flex-col items-center justify-center">
               ...
               <h3 class="font-black text-lg text-gray-900 uppercase mb-2">ĐĂNG KÝ NHẬN BẢN TIN</h3>
               <p class="text-xs text-gray-600 mb-4">Không bỏ lỡ những sự kiện và tin tức độc quyền từ chúng tôi.</p>
               <div class="flex w-full bg-white rounded-full p-1 border border-brand-200 shadow-sm">
                 <input type="email" placeholder="Email của bạn" class="flex-1 bg-transparent border-none outline-none text-sm px-4 text-gray-900" />
      ...
      <section>
        <div class="mb-10 flex items-center gap-4 border-b border-gray-100 pb-4" data-aos="fade-up">
           <h2 class="text-2xl font-black text-gray-900 uppercase tracking-widest">
             <i class="pi pi-list mr-2 text-brand-600"></i> BÀI VIẾT KHÁC
           </h2>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-8 lg:gap-10">
          ...
          <div class="w-full lg:w-1/3 xl:w-1/4">
             <div class="bg-[#dcf0fc] rounded-xl overflow-hidden shadow-sm transition-all duration-300 sticky top-28" data-aos="fade-right">
                ...
                >
                  <h3 class="font-extrabold text-[#2a3754] tracking-wide text-[15px] uppercase">MỤC LỤC NỘI DUNG</h3>
                  ...
                  <div v-show="isTocOpen" class="border-t border-[#b8dcf5]">
                    <ul class="p-4 md:px-5 py-3 flex flex-col gap-4 max-h-[60vh] overflow-y-auto custom-scrollbar">
                      <li v-for="(item, index) in tocItems" :key="index">
                        <a href="#" class="text-[#4581e6] font-bold text-[14px] leading-relaxed hover:text-[#18449c] transition-colors block py-1">
                          {{ item }}
                        </a>
                      </li>
                    </ul>
             ...
             <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               ...
               <div v-for="(post, idx) in postStore.posts" :key="post.id" @click="goToPost(post.slug)" class="bg-white rounded-[30px] p-6 shadow-sm border border-gray-100 group cursor-pointer hover:shadow-[0_20px_50px_rgba(220,38,38,0.1)] hover:-translate-y-2 hover:border-brand-200 transition-all duration-500 flex flex-col" data-aos="fade-up" :data-aos-delay="idx % 3 * 100">
                  <div class="w-full aspect-video rounded-[20px] overflow-hidden relative mb-6">
                     ...
                     <div class="absolute bottom-4 left-4 bg-white/90 backdrop-blur text-gray-900 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-wider shadow-sm">Tin Sự Kiện</div>
                     <div class="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                  </div>
                  ...
                  <p class="text-sm text-gray-500 line-clamp-3 mb-6 block flex-1">Bài viết cung cấp những thông tin chuyên sâu và góc nhìn mới mẻ về ngành tổ chức sự kiện hiện nay.</p>
                  ...
                  <div class="border-t border-gray-100 pt-4 mt-auto">
                    <span class="text-xs font-black text-brand-600 uppercase tracking-widest flex items-center gap-2 group-hover:gap-4 transition-all duration-300">
                      ĐỌC TIẾP <i class="pi pi-arrow-right transition-transform group-hover:translate-x-1"></i>
                    </span>
                  </div>
               </div>
             </div>
...
const changePage = async (page: number) => {
  await postStore.fetchPosts({ page, limit: 9 })
...
const tocItems = [
  'Vai Trò Của Kick Off Meeting Đối Với Doanh Nghiệp',
  'Các Hình Thức Tổ Chức Kick Off Meeting Phổ Biến',
  'Nội Dung Chương Trình Kick Off Meeting',
  'Quy Trình Tổ Chức Kick Off Meeting Doanh Nghiệp',
  'Cyber Show: Đơn Vị Tổ Chức Kick Off Meeting Chuyên Nghiệp',
  'FAQ: Kick Off Meeting'
]
</script>
